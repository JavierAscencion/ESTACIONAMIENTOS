...
const char ESTATUS_PAS = CAN_MODULE[0] == 'E'? 'O':'I';//MODULE E Ó V, ESTADO PARA SALIR O ENTRRAR
const char ESTATUS_MOD = CAN_MODULE[0] == 'E'? 'I':'O';//MODULE E Ó V, EVENTO GENERADO POR ENTRAR O SALIR
...
void usart_user_read_text(){
  ...
  if(!usart.rx_overflow){
    ...
    if(!(tam == 9 || tam == 7)){
      ...
      #ifdef debug_usart
      usart_write_line("Key NO cumple con el tamaño");
      #endif
      return;
    ...
    //Valida respecto al formato leido, fol ó fecha
    if(tam == 7){
    ...
    #ifdef debug_usart
    usart_write_line("RS232: Dato dañado, overflow");
    #endif
  }
}
/******************************************************************************/
